...
export default function SignupPage() {
  ...
  return (
    <div className="flex min-h-screen bg-background font-body-md text-text-primary antialiased p-4 sm:p-gutter items-center justify-center">
          ...
          <div className="mx-auto w-full max-w-md">
            ...
            {stage === 'verify-sent' ? (
              <div className="bg-surface border border-border-base rounded-2xl p-6 sm:p-8 shadow-sm relative z-10 text-center">
                ...
                <h2 className="font-headline-md text-headline-md text-text-primary mb-2 font-bold">Check your inbox</h2>
                ...
                <form onSubmit={async (e) => {
                  ...
                  <input 
                    ...
                    className="w-full text-center tracking-widest text-2xl bg-surface border border-border-base text-text-primary focus:border-primary focus:ring-1 focus:ring-primary rounded-xl py-3 font-body-md transition-shadow min-h-[44px]" 
                  />
                  <button 
                    type="submit" 
                    disabled={loading || verificationCode.length < 6}
                    className="w-full py-3.5 rounded-xl bg-primary-container text-on-primary font-label-md text-label-md font-bold hover:bg-primary transition-colors disabled:opacity-50 flex items-center justify-center min-h-[44px]"
                  >
                  ...
                  <button 
                    ...
                    disabled={resending}
                    className="w-full mt-2 font-label-md text-label-md text-primary hover:underline flex justify-center items-center min-h-[44px]"
                  >
              ...
              <>
                <div className="text-center lg:text-left mb-6">
                  <h2 className="font-display-lg-mobile lg:font-headline-lg text-display-lg-mobile lg:text-headline-lg text-text-primary tracking-tight font-extrabold">
                    Create your account
                  </h2>
                  <p className="mt-2 font-body-md text-body-md text-text-secondary">
                    Already have an account? <Link className="font-label-md text-label-md text-primary hover:text-primary-container font-semibold transition-colors min-h-[44px] inline-flex items-center" to="/login">Sign in here</Link>
                  </p>
                </div>
                ...
                <div className="bg-surface border border-border-base rounded-2xl p-6 sm:p-8 shadow-sm relative z-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <button onClick={handleGoogleSignup} disabled={loading || googleLoading || githubLoading} className="flex items-center justify-center w-full px-4 py-3 border border-border-base rounded-xl bg-surface hover:bg-surface-container-low transition-colors duration-200 disabled:opacity-50 group min-h-[44px]">
                    ...
                    <button onClick={handleGitHubSignup} disabled={loading || googleLoading || githubLoading} className="flex items-center justify-center w-full px-4 py-3 border border-border-base rounded-xl bg-surface hover:bg-surface-container-low transition-colors duration-200 disabled:opacity-50 group min-h-[44px]">
                  ...
                  <div className="relative mb-4">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-2 bg-surface text-text-secondary font-label-sm text-label-sm font-medium">Or continue with email</span>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleSignup} className="space-y-4" autoComplete="off">
                    ...
                    <div>
                      <label className="block font-label-md text-label-md text-text-primary mb-1 font-semibold" htmlFor="name">Full Name</label>
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                          <span className="material-symbols-outlined text-text-secondary text-xl">person</span>
                        </div>
                        <input 
                          className="block w-full pl-10 pr-3 py-3 border border-border-base rounded-xl text-text-primary bg-surface focus:outline-none focus:ring-2 focus:ring-primary-container focus:border-primary-container font-body-md text-body-md placeholder-text-secondary transition-shadow min-h-[44px]" 
                          id="name" 
                    ...
                    <div>
                      <label className="block font-label-md text-label-md text-text-primary mb-1 font-semibold" htmlFor="email">Work Email</label>
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    ...
                    <div>
                      <label className="block font-label-md text-label-md text-text-primary mb-1 font-semibold" htmlFor="password">Password</label>
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                          <span className="material-symbols-outlined text-text-secondary text-xl">lock</span>
                        </div>
                        <input 
                          className="block w-full pl-10 pr-10 py-3 border border-border-base rounded-xl text-text-primary bg-surface focus:outline-none focus:ring-2 focus:ring-primary-container focus:border-primary-container font-body-md text-body-md placeholder-text-secondary transition-shadow min-h-[44px]" 
                          id="password" 
                          placeholder="••••••••" 
                          required 
                        ...
                        />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-text-secondary hover:text-primary transition-colors min-w-[44px] justify-center">
                    ...
                    <div className="flex items-center mt-4">
                      <input 
                        className="h-5 w-5 text-primary-container focus:ring-primary-container border-border-base rounded text-primary" 
                        id="terms" 
                        required 
                        type="checkbox"
                        checked={agreed}
                      ...
                      />
                      <label className="ml-2 block font-body-sm text-body-sm text-text-secondary cursor-pointer select-none py-2" htmlFor="terms">
                        I agree to the <Link className="text-primary hover:underline font-semibold" to="/terms">Terms of Service</Link> and <Link className="text-primary hover:underline font-semibold" to="/privacy">Privacy Policy</Link>
                      </label>
                    </div>

                    <div className="pt-2">
                      <button disabled={loading} className="w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-sm font-label-md text-label-md text-on-primary bg-primary-container hover:bg-primary font-bold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-container transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 min-h-[44px]" type="submit">
